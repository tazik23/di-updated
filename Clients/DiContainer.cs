...
{
    public static IContainer BuildContainer(CommandLineArgs args)
    {
        ...
        builder.Register(ctx =>
        {
            ...
            return new CloudSettings
            {
                ...
                LayoutOptions = new LayoutSettings
                {
                    ...
                    SpiralStep = cmdArgs.SpiralStep ?? 0.1,
                    SpiralAngleStep = cmdArgs.SpiralAngleStep ?? 0.1
                },
        ...
        builder.RegisterType<TextProcessor>().As<ITextProcessor>()
            .WithParameter(
                (pi, ctx) => pi.Name == "normalizers",
                (pi, ctx) => new[] { ctx.Resolve<IWordNormalizer>() })
            .WithParameter(
                (pi, ctx) => pi.Name == "filters",
                (pi, ctx) => new[] { ctx.Resolve<IWordFilter>() });
        ...
        builder.Register(ctx =>
        {
            var settings = ctx.Resolve<LayoutSettings>();
            return new ArchimedeanSpiral(
                settings.Center);
        }).As<ISpiral>();
